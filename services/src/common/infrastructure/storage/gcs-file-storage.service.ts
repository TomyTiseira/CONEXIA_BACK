 ...
 * Used in production environments to store service images in GCS.
 *
...
export class GCSFileStorage implements FileStorage {
  ...
  async upload(file: Buffer, path: string, mimetype: string): Promise<string> {
    try {
      ...
      console.error('[GCS Services] Upload error:', error);
  ...
  async delete(path: string): Promise<void> {
    try {
      ...
      console.warn(
        `Failed to delete service image ${path} from GCS:`,
        error.message,
      );
    }
  }
