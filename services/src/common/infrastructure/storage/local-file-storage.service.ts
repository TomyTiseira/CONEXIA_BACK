 ...
 *
 * Files are stored in the local 'uploads/services' directory.
 */
@Injectable()
export class LocalFileStorage implements FileStorage {
  private readonly uploadDir: string;

  constructor(baseDir: string = 'uploads/services') {
  ...
  async upload(file: Buffer, path: string): Promise<string> {
    ...
    // Return the relative path (filename only for backwards compatibility)
    return path;
  }
  ...
  async delete(path: string): Promise<void> {
    try {
      ...
      console.warn(`Failed to delete service image ${path}:`, error.message);
    }
  }
