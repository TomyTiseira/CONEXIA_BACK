...
const fileStorageProvider = {
  ...
  useFactory: (): FileStorage => {
    ...
    if (isProduction) {
      ...
      if (!envs.gcs.projectId || !envs.gcs.serviceImagesBucket) {
        throw new RpcException(
          'GCS configuration is required in production for service images',
        );
      }
      ...
      return new GCSFileStorage(
        envs.gcs.projectId,
        envs.gcs.keyFile || '',
        envs.gcs.serviceImagesBucket,
      );
    } else {
      // Development: Use local filesystem
      console.log('Using local file storage for service image uploads');
