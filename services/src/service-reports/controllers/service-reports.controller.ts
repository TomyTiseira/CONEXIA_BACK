...
export class ServiceReportsController {
  ...
  async createServiceReport(
    @Payload() data: { createServiceReportDto: CreateServiceReportDto; userId: number },
  ) {
  ...
  async getServicesWithReports(
    ...
    return await this.getServicesWithReportsUseCase.execute(getServiceReportsListDto);
  }
}
